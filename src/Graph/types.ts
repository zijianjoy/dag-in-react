...
import {
  ...
  STATE_PRIORITY,
  TextIconConfig,
} from "./data_types_internal";

/**
 * Expose internal shared Properties
 */
export { baseColors, convertStateToRuntime, isNoState, isTextIcon };
export type {
  EdgeStyle,
  IconConfig,
  MarkerStyle,
...
export function repeatedMetaNodes(
  ids: string[],
  config: DagMeta | ((id: string) => DagMeta)
) {
...
export function isDagreInit(
  ...
  nodeResolver?: (id: string) => DagNode | DagGroup
): boolean {
...
export interface RawDagNodeSkeleton {
  id: string;
  /**
...
type GetOptionalParamsFor<Class> = Partial<
  Pick<
    Class,
    Exclude<
      keyof Class,
...
export class DagNode
  implements
    ...
    PartialBy<Required<DagNodeMeta>, "icon">,
    Point,
    Dimension
{
  id: string = "";
   ...
   *
   * Default type coersion is to retain type, unless the source is a group. In
  ...
  static from(node: DagNode | DagGroup, coerceType?: NodeType) {
    ...
    return new DagNode(node.id, coerceType || "execution", node.state, {
      ...node,
    });
  }
  ...
  static bootstrapSkeletonNode(
    node: DagNodeSkeleton,
    nodeMeta: DagNodeMeta
  ): DagNode;
  static bootstrapSkeletonNode(
    node: DagNodeSkeleton,
    nodeMeta: DagCustomNodeMeta
  ): CustomNode;
  static bootstrapSkeletonNode(
    node: DagNodeSkeleton,
    nodeMeta: DagGroupMeta
  ): DagGroup;
  static bootstrapSkeletonNode(node: DagNodeSkeleton, nodeMeta: DagMeta) {
    ...
    assertCompleteDereference(
      parentRemains,
      `Unexpected keys found in DagNodeSkeleton`,
    ...
    if (type !== "group") {
      ...
      assertCompleteDereference(
        remains,
        "Not all fields of DagNode were assigned via Skeleton"
      );
    ...
    assertCompleteDereference(
      remains,
      "Not all fields of DagGroup were assigned via Skeleton",
      ["groupMeta"]
    );
 ...
 * content on the page (design: http://go/dag-component#heading=h.somk3w98o2gr)
 *
...
export class CustomNode extends DagNode implements CustomNodeProps {
  ...
  constructor(
    ...
    this.width = width;
    this.height = height;
    Object.assign(this, { includeInStepCount, hideEdgeMarkers });
  }
...
function createFromSkeleton(
  ...
  while (queue.length) {
    ...
    if (
      !(node instanceof DagNode) &&
      skeletonIsGroup(skNode) &&
      skNode.definition
    ) {
      const groupMeta = nodeMeta as DagGroupMeta;
      const d = DagNode.createFromSkeleton(
        skNode.definition,
        groupMeta.groupMeta
      );
      node.edges = d.edges;
